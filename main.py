...
iv = None
region = None
current_chat_id = None  # <-- متغير جديد لتخزين chat_id الحالي
...
async def process_invite_command(team_code: str, target_uid: str, key, iv, region, chat_id: int, status_msg_id: int, user_id: int):
    try:
        ...
        await SEndPacKeT(whisper_writer, online_writer, 'OnLine', join_packet)
        await asyncio.sleep(1.5)
        invite_packet = await SEnd_InV(5, int(target_uid), key, iv, region)
...
async def process_ms_command(team_code: str, user_message: str, key, iv, region, chat_id: int, status_msg_id: int, user_id: int):
    ...
    try:
        ...
        # انتظار حتى يتم تحديث current_chat_id
        timeout = 10
        waited = 0
        saved_chat_id = None
        while waited < timeout:
            if current_chat_id is not None:
                saved_chat_id = current_chat_id
                break
            await asyncio.sleep(0.5)
            waited += 0.5
        ...
        for i in range(4):
            ...
            await safe_send_message(0, colored_message, user_id, saved_chat_id, key, iv)
            await asyncio.sleep(0.5)
        ...
        # إعادة تعيين current_chat_id (اختياري)
        current_chat_id = None
